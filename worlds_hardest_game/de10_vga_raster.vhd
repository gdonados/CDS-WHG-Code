-------------------------------------------------------------------------------
--
-- Author: Gabe Donados, Erik Grundeland
...
architecture rtl of de2_vga_raster is
	...
	--'Game state' signals end of level conditions
	signal touched_goal : std_logic := '0';
	
	--Background generation bits
	signal mapE : unsigned(1 downto 0) := "00";	-- map select
...
begin
			...
			--Addressing for read communication of Avalon bus
				...
				if read = '1' then
					if address= "0000000000000000" then		--0
						...
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_2_y); --Read y position of enemy sprite 2
					elsif address = "0000000000001000" then --8
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_2_x); --Read x position of enemy sprite 2
					elsif address = "0000000000001001" then --9
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_3_y); --Read y position of enemy sprite 3
					elsif address = "0000000000001010" then --10
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_3_x); --Read x position of enemy sprite 3
					elsif address = "0000000000001011" then --11
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_4_y); --Read y position of enemy sprite 4
					elsif address = "0000000000001100" then --12
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_4_x); --Read x position of enemy sprite 4
					elsif address = "0000000000001101" then --13
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_5_y); --Read y position of enemy sprite 5
					elsif address = "0000000000001110" then --14
						readdata <=	 "000000" & std_logic_vector(enemy_sprite_5_x); --Read x position of enemy sprite 5
					else 
				...
				if write = '1' then
					...
					if address = "0000000000100010" then --34		--Write level
						mapE <= unsigned(writedata(1 downto 0));
					end if;
				end if;
			end if;
		end if;
	...
	begin
		...
		if (mapE = "00") then
			...
			-------------------------------------------------------------------------------------- SECTION 2 (start/end zones)
			elsif (Vcount >= 205 and Vcount <= 210) then
					if(Hcount < 249) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >=249 and Hcount <= 331) then	--top boarder for start zone 
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 331 and Hcount < 599) then			--center gap
							VGA_R <= "11111111";	--white
							VGA_B <= "11111111";
							VGA_G <= "11111111";							
					elsif (Hcount >= 599 and Hcount <= 693)	then	-- top boarder for end zone
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 693)	then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					end if;
					
			elsif (Vcount > 210 and Vcount < 305) then
					if(Hcount < 249) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 254)	then --first start boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 254 and Hcount < 604)	then --white for start section						 
							VGA_R <= "11111111";	--white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 604 and Hcount < 688)	then --green for end section
							VGA_R <= "00000000";	--green
							VGA_B <= "00000000";
							VGA_G <= "11111111";
					elsif (Hcount >=688 and Hcount <=693) then	--end boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 693)	then
				...
				elsif (Vcount >= 305 and Vcount <= 310) then
					if(Hcount < 249) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >=249 and Hcount <= 331) then	--bottom boarder for start zone 
							VGA_R <= "00000000";		--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 331 and Hcount < 599)	then		--center gap
							...
							VGA_B <= "11111111";
							VGA_G <= "11111111";	
					elsif (Hcount >= 599 and Hcount <= 693)	then -- bottom boarder for end zone
							VGA_R <= "00000000";		--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 693)	then
...
-------------------------------------------------------------------------------------- SECTION 3 (center square)

			elsif (Vcount >= 135 and Vcount <= 140) then
					if(Hcount < 324) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 324 and Hcount <= 604)	then --top boarder for middle
							VGA_R <= "00000000";		--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					end if;

			elsif (Vcount > 140 and Vcount < 254) then							
					if (Hcount < 324) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 324 and Hcount <= 331)	then --left top boarder for middle
							VGA_R <= "00000000";		--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 331 and Hcount < 599)	then
							VGA_R <= "11111111";    -- white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 599 and Hcount <= 604)	then --right top boarder for middle
							VGA_R <= "00000000";		--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604) then
			...
			elsif (Vcount > 310 and Vcount < 409) then							
					if (Hcount < 324) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 324 and Hcount <= 331)	then--left bottom boarder for middle
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 331 and Hcount < 599)	then
							...
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 599 and Hcount <= 604)	then--right bottom boarder for middle
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604) then
			...
			elsif (Vcount >= 409 and Vcount <= 414) then
					if(Hcount < 324) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 324 and Hcount <= 604)	then--bottom boarder for middle
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604) then
...
-------------------------------------------------------------------------------------- Space after map
			elsif (Vcount > 414) then        
					VGA_R <= "00000000";    -- blue
					VGA_B <= "01111111";
					VGA_G <= "00000000";				
			end if;	--first map end
-------------------------------------------------------------------------------------- Space before map
-------------------------------------------------------------------------------------- Space before map
			...
			-------------------------------------------------------------------------------------- SECTION 2 (start/end zones)
					...
					if(Hcount < 249) then
							...
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif(Hcount >= 249 and Hcount <= 254) then	--start boarder
							VGA_R <= "00000000";    -- black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif(Hcount > 254 and Hcount < 374) then	--start zone
							VGA_R <= "11111111";    -- white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >=374 and Hcount <= 604) then	--center bottom boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604 and Hcount < 688) then	--map zone
							VGA_R <= "11111111";	--white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 688 and Hcount <= 693) then	-- end boarder
					...
					if(Hcount < 249) then
							...
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 254 and Hcount < 374) then	--white for start section						
							VGA_R <= "11111111";	--white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 374 and Hcount <= 381) then	--left center boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 381 and Hcount < 599) then	-- center gap
							VGA_R <= "00000000";	--blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >=599 and Hcount <= 604) then	--right center boarder 
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 604 and Hcount < 688) then	--green for end section
					...
					if(Hcount < 249) then
							...
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >=249 and Hcount <= 381) then	--bottom boarder for start zone  
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif(Hcount >381 and Hcount <599)	then		--center gap
			...
			elsif (Vcount >= 105 and Vcount <= 110) then
					if(Hcount < 249) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 693) then	--top boarder for middle
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 693) then
			...
			elsif (Vcount > 110 and Vcount < 205) then						
					if (Hcount < 249) then
							VGA_R <= "00000000";    -- blue
							VGA_B <= "01111111";
							VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 254) then	--left boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 254 and Hcount < 688) then	--map space
							VGA_R <= "11111111";    -- white
							VGA_B <= "11111111";
							VGA_G <= "11111111";
					elsif (Hcount >= 688 and Hcount <= 693) then	--right boarder
							VGA_R <= "00000000";	--black
							VGA_B <= "00000000";
							VGA_G <= "00000000";
					elsif (Hcount > 693) then
			...
			elsif (Vcount > 310) then              
					VGA_R <= "00000000";    -- blue
					VGA_B <= "01111111";
					VGA_G <= "00000000";
-------------------------------------------------------------------------------------- END SECTION 4

			end if;	-- second map end
			...
			if (Vcount < 205) then
					...
					VGA_B <= "01111111";
					VGA_G <= "00000000";
			elsif (Vcount >=205 and Vcount <= 210) then	--top boarder
					if (Hcount < 249) then
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 381) then	--starting top boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 381 and Hcount < 391) then	--center gap
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 391 and Hcount <= 693) then	--ending top boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 693) then
				...
				elsif (Vcount > 210 and Vcount < 305)	then--center zone
					if (Hcount < 249) then 
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";	
						...
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 245 and Hcount < 374) then	--start zone
						VGA_R <= "11111111";    -- white
						VGA_B <= "11111111";
						VGA_G <= "11111111";
					elsif (Hcount >= 374 and Hcount <=381) then	--inside start boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 381 and Hcount < 391) then	--inside gap
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 391 and Hcount <= 396) then	--end middle boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 391 and Hcount <= 441) then	--end green zone
						VGA_R <= "00000000";    -- green
						VGA_B <= "00000000";
						VGA_G <= "11111111";
					elsif (Hcount >= 441 and Hcount < 688)	then--zone before end zone
						VGA_R <= "11111111";    -- white
						VGA_B <= "11111111";
						VGA_G <= "11111111";
					elsif (Hcount >= 688 and Hcount <= 693) then --end boarder
			...
			elsif (Vcount >=305 and Vcount <= 310) then	--middle-bottom boarder
					if (Hcount < 249) then
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 254) then	--starting bottom boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 254 and Hcount < 374) then	--starting gap
						VGA_R <= "11111111";    -- white
						VGA_B <= "11111111";
						VGA_G <= "11111111";
					elsif (Hcount >= 374 and Hcount <= 441) then	--starting middle boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 441 and Hcount < 688) then	--map space
						VGA_R <= "11111111";    -- white
						VGA_B <= "11111111";
						VGA_G <= "11111111";
					elsif (Hcount >= 688 and Hcount <= 693) then	--end boarder
				...
				elsif (Vcount > 310 and Vcount < 409) then	-- center zone
					if (Hcount < 249) then
						VGA_R <= "00000000";    -- blue
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 254) then 	--first boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 254 and Hcount < 688) then 	--map zone
						VGA_R <= "11111111";    -- white
						VGA_B <= "11111111";
						VGA_G <= "11111111";
					elsif (Hcount >= 688 and Hcount <= 693) then --end boarder
					...
					if (Hcount < 249) then
						...
						VGA_B <= "01111111";
						VGA_G <= "00000000";
					elsif (Hcount >= 249 and Hcount <= 693) then 	--bottom boarder
						VGA_R <= "00000000";    -- black
						VGA_B <= "00000000";
						VGA_G <= "00000000";
					elsif (Hcount > 693) then 
	...
	--Combinational logic for VGA signals
	VGA_CLK <= clk_25;
